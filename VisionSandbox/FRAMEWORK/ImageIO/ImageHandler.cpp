...
bool ImageHandler::injectImage(cv::Mat image)
{
//    if (m_grayImage != NULL)
//    {
//        cvReleaseImage(&m_grayImage);
...
//    }
    if (image.channels() > 1)
    {
        cv::Mat gray; cv::cvtColor(image, gray, CV_BGR2GRAY);
        m_rgbImageNoPointer = image.clone();
        m_rgbImage = &m_rgbImageNoPointer;
        m_grayImageNoPointer = gray.clone();
        m_grayImage = &m_grayImageNoPointer;
        m_imgSrc->curImage = m_rgbImage;
        //        cvCvtColor2(m_imgSrc->curImage, m_grayImage, CV_RGB2GRAY);
        //        m_rgbImage = cvCloneImage(m_imgSrc->curImage);
    }
    else
    {
        m_grayImageNoPointer = image.clone();
        m_grayImage = &m_grayImageNoPointer;
        m_rgbImage = NULL;
        m_imgSrc->curImage = m_grayImage;
    }
    return true;
}
...
void ImageHandler::viewImage(char* name, int autoresize)
{
    ...
    if (getImageSize2().width < 75)
    {
        cvResizeWindow(name, 75, 75/getImageSize2().width*getImageSize2().height);
    }
...
void ImageHandler::paintRectangle(CvRect rect, Color2 color, int thickness, bool filled)
{
...
void ImageHandler::paintRectangle(Rect2 rect, Color2 color, int thickness, bool filled)
{
...
void ImageHandler::paintCenter(Rect2 rect, Color2 color, int thickness)
{
	CvPoint pt = { static_cast<int>(rect.left + floor(rect.width/2+0.5)), static_cast<int>(rect.upper + floor(rect.height/2+0.5)) };

    cvRectangle(m_imgSrc->curImage, pt, pt, CV_RGB(color.red, color.green, color.blue), thickness*2);
}

void ImageHandler::paintLine(Point2D p1, Point2D p2, Color2 color, int thickness)
{
...
void ImageHandler::paintCircle(Point2D center, int radius, Color2 color, int thickness)
{
...
void ImageHandler::paintPoint(Point2D center, Color2 color, int thickness)
{
...
Size2 ImageHandler::getImageSize2()
{
	return m_imgSrc->getImageSize2();
}
...
void ImageHandler::putTextOnImage(char* text, Point2D org, Color2 color, float fontSize2)
{
	CvFont font;
	cvInitFont( &font, CV_FONT_VECTOR0, fontSize2, fontSize2, 0, 1, 20);
	cvPutText( m_imgSrc->curImage, text, cvPoint(org.col,org.row), &font, cvScalar(color.blue,color.green,color.red) );
}


void ImageHandler::saveROIofGrayImage(Rect2 rect, char* filename)
{
...
unsigned char* ImageHandler::getPatch(Rect2 rect)
{
    int iplCols = m_grayImage->widthStep;
