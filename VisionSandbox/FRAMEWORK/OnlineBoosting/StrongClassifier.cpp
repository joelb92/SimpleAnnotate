...
float StrongClassifier::getFeatureValue(ImageRepresentation *image, Rect2 ROI, int baseClassifierIdx)
{
	return baseClassifier[baseClassifierIdx]->getValue(image, ROI);
}


float StrongClassifier::eval(ImageRepresentation *image, Rect2 ROI)
{
	float value = 0.0f;
...
bool StrongClassifier::update(ImageRepresentation *image, Rect2 ROI, int target, float importance)
{
	assert (true);
	return false;
}

bool StrongClassifier::updateSemi(ImageRepresentation *image, Rect2 ROI, float priorConfidence)
{
	assert (true);
	return false;
}
...
float StrongClassifier::getImportance(ImageRepresentation *image, Rect2 ROI, int target, int numBaseClassifier)
{
