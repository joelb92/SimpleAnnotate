...
StrongClassifierDirectSelection::StrongClassifierDirectSelection(int numBaseClassifier, int numWeakClassifier, 
																 Size2 patchSize2, bool useFeatureExchange, int iterationInit) 
																 : StrongClassifier(numBaseClassifier, numWeakClassifier, patchSize2, useFeatureExchange, iterationInit)
{
	this->useFeatureExchange = useFeatureExchange;
	baseClassifier = new BaseClassifier*[numBaseClassifier];
	baseClassifier[0] = new BaseClassifier(numWeakClassifier, iterationInit, patchSize2);
...
bool StrongClassifierDirectSelection::update(ImageRepresentation *image, Rect2 ROI, int target, float importance)
{
	...
	{
		int replacedClassifier = baseClassifier[0]->replaceWeakestClassifier (m_sumErrors, patchSize2);
