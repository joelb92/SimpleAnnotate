...
public:
	...
	BaseClassifier(int numWeakClassifier, int iterationInit, Size2 patchSize2); 
	BaseClassifier(int numWeakClassifier, int iterationInit, WeakClassifier** weakClassifier); 

	virtual ~BaseClassifier();

	void trainClassifier(ImageRepresentation* image, Rect2 ROI, int target, float importance, bool* errorMask);
	
	void getErrorMask(ImageRepresentation* image, Rect2 ROI, int target, bool* errorMask);
	void getErrors(float* errors);
	virtual int selectBestClassifier(bool* errorMask, float importance, float* errors); 
	
	virtual int replaceWeakestClassifier(float* errors, Size2 patchSize2);
	...
	float getValue(ImageRepresentation *image, Rect2 ROI, int weakClassifierIdx = -1);
...
protected:
	...
	float* m_wWrong;
	int m_iterationInit;
	void generateRandomClassifier (Size2 patchSize2);
