...
BaseClassifier::BaseClassifier(int numWeakClassifier, int iterationInit, Size2 patchSize2)
{
	...
	generateRandomClassifier(patchSize2);
...
void BaseClassifier::generateRandomClassifier (Size2 patchSize2)
{
	for (int curWeakClassifier = 0; curWeakClassifier< m_numWeakClassifier+m_iterationInit; curWeakClassifier++)
	{
		weakClassifier[curWeakClassifier] = new WeakClassifierHaarFeature(patchSize2);
	}
}

int BaseClassifier::eval(ImageRepresentation *image, Rect2 ROI)
{
	return weakClassifier[m_selectedClassifier]->eval(image, ROI);
}

float BaseClassifier::getValue (ImageRepresentation *image, Rect2 ROI, int weakClassifierIdx)
{
...
void BaseClassifier::trainClassifier(ImageRepresentation* image, Rect2 ROI, int target, float importance, bool* errorMask)
{
	//get poisson value
	double A = 1;
	int K = 0;
...
void BaseClassifier::getErrorMask(ImageRepresentation* image, Rect2 ROI, int target, bool* errorMask)
{
	for (int curWeakClassifier = 0; curWeakClassifier < m_numWeakClassifier+m_iterationInit; curWeakClassifier++)
...
int BaseClassifier::replaceWeakestClassifier(float* errors, Size2 patchSize2)
{
	float maxError = 0.0f;
	int index = -1;
	...
	if (maxError > errors[m_idxOfNewWeakClassifier])
	{
		...
		weakClassifier[m_idxOfNewWeakClassifier] = new WeakClassifierHaarFeature (patchSize2);
