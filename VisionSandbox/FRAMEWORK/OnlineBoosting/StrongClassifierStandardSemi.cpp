																	  ...
																	  iterationInit)
{
	...
	{
		baseClassifier[curBaseClassifier] = new BaseClassifier(numWeakClassifier, iterationInit, patchSize2);
	}
...
bool StrongClassifierStandardSemi::updateSemi(ImageRepresentation *image, Rect2 ROI, float priorConfidence)
{
	...
	for (curBaseClassifier = 0; curBaseClassifier<numBaseClassifier; curBaseClassifier++)
	{
		...
		int selectedClassifier;
		{
			...
			int myTarget = static_cast<int>(signbit(combinedDecision));
		...
		if (useFeatureExchange)
			baseClassifier[curBaseClassifier]->replaceWeakestClassifier (m_errors, patchSize2);
